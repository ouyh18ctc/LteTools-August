...
namespace Lte.Domain.Test.LinqToExcel
{
    ...
    public class ColumnMappings_IntegrationTests : SQLLogStatements_Helper
    {
        ...
        public void log_warning_when_property_with_column_mapping_not_in_where_clause_when_mapped_column_doesnt_exist()
        {
            ...
            int warningsLogged = _loggedEvents.GetEvents().Count(logEvent => 
                (logEvent.Level == Level.Warn) 
                && (logEvent.RenderedMessage 
                == "'The Big Cheese' column that is mapped to the 'CEO' property does not exist in the 'ColumnMappings' worksheet"));
